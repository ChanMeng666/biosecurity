...
def admin_manage_staff():
    ...
    if 'flash_message' in request.args and request.args.get('flash_message') == 'staff_not_selected':
...
@admin_bp.route('/edit-staff', methods=['GET'])
def edit_staff():
    if 'admin_info' not in session:
        return redirect(url_for('auth.login'))

    selected_staff_id = request.args.get('selected_staff_id')
    if selected_staff_id:
        connection = get_db_connection()
        cursor = connection.cursor(dictionary=True)
        try:
        ...
        if staff_member:
            return render_template('admin/admin_manage_staff_edit.html', staff_member=staff_member, show_offcanvas=True)
        else:
...
def update_staff():
    ...
    user_id = request.form.get('edit_staff_id')
    username = request.form.get('edit_staff_username')
    password = request.form.get('edit_staff_password')
    first_name = request.form.get('edit_staff_first_name')
    last_name = request.form.get('edit_staff_last_name')
    email = request.form.get('edit_staff_email')
    phone = request.form.get('edit_staff_phone')
    hire_date = request.form.get('edit_staff_hire_date')
    position = request.form.get('edit_staff_position')
    department = request.form.get('edit_staff_department')

    if password and not is_password_complex(password):
        flash('Password does not meet complexity requirements.', 'danger')
        return redirect(url_for('admin.edit_staff', selected_staff_id=user_id))
    ...
    try:
        ...
        cursor.execute("""
            UPDATE staff_and_administrators SET first_name = %s, last_name = %s,
            email = %s, work_phone_number = %s, hire_date = %s,
            position = %s, department = %s
            WHERE user_id = %s
        """, (first_name, last_name, email, phone, hire_date, position, department, user_id))

        connection.commit()
        flash('Staff member information updated successfully.', 'success')
    except mysql_errors.Error as e:
        connection.rollback()
