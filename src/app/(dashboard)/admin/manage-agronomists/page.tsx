...
export default function ManageAgronomistsPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<Agronomist | null>(null);
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    const res = await fetch(`/api/agronomists/${deleteTarget.agronomistId}`, { method: "DELETE" });
    if (res.ok) {
      toast.success("Agronomist deleted");
      fetchData();
    } else {
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTable
        ...
        loading={loading}
        actions={(item: Agronomist) => (
          <div className="flex gap-1">
            <Link href={`/admin/manage-agronomists?edit=${item.agronomistId}`} className={cn(buttonVariants({ variant: "ghost", size: "icon" }), "h-8 w-8")}>
