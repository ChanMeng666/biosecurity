...
export default function ManageGuidesPage() {
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    const res = await fetch(`/api/guides/${deleteTarget.agricultureId}`, { method: "DELETE" });
    if (res.ok) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Link href="/admin/manage-guides/add" className={cn(buttonVariants(), "bg-emerald-600 hover:bg-emerald-700")}>
      ...
      <DataTable
        ...
        actions={(item: Guide) => (
          <div className="flex gap-1">
            <Link href={`/admin/manage-guides/${item.agricultureId}/edit`} className={cn(buttonVariants({ variant: "ghost", size: "icon" }), "h-8 w-8")}>
      ...
      <ConfirmDialog
        ...
        title="Delete Guide"
        description={`Are you sure you want to delete "${deleteTarget?.commonName}"? This action cannot be undone.`}
