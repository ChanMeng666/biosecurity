...
@staff_bp.route('/staff/manage-guide/edit/<int:agriculture_id>', methods=['GET'])
def staff_manage_guide_edit(agriculture_id):
    connection = get_db_connection()
    cursor = connection.cursor(dictionary=True)
    ...
    return render_template(
        'staff/staff_manage_guide_edit.html',
        agriculture_item=agriculture_item,
        images=images
    )

@staff_bp.route('/staff/manage-guide/update/<int:agriculture_id>', methods=['POST'])
def staff_manage_guide_update(agriculture_id):
    # Get form data
    form_data = request.form.to_dict()
